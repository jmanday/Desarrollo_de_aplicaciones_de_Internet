...
urls = ( '/', 'index',
	 ...
	 '/maps','maps',
	 '/twitter','twitter',
	 '/twitter_estado','twitter_estado',
	 '/buscar_contenido_twitter','buscar_contenido_twitter',
	 '/mostrar_mis_tweets','mostrar_mis_tweets',
	 '/(.*)', 'error'
       )
...
consumer_key = 'cld8zgdZak94HuZmdKqiNCZmh'
consumer_secret = 'uCxagjDRmilEwjoOTMfCreCL4BIMzQ98UJlopuFhUdQWOgAeNG'
access_token = '166547820-KcrjZCloodC8QxJtKYq3HJ0h9HEmYpYvvVSiJBBo'
access_token_secret = 'avktRlJcxjLi55ovA5tk3MaFz8xVCr5M3RdFX8FLImZMA'
...
class twitter:
	def GET(self):
		usuario = comprueba_usuario()
		web.header('Content-Type', 'text/html; charset=utf-8')
		return plantillas.twitter()

class twitter_estado:
	def GET(self):
		usuario = comprueba_usuario()
		formulario_nuevo_estado = form_twitter1()
		web.header('Content-Type', 'text/html; charset=utf-8')
		return plantillas.nuevo_estado_twitter(mensaje='',reg=formulario_nuevo_estado.render())

	def POST(self):
		usuario = comprueba_usuario()
		formulario_nuevo_estado = form_twitter1()

		if not formulario_nuevo_estado.validates():
			web.header('Content-Type', 'text/html; charset=utf-8')
			return plantillas.nuevo_estado_twitter(mensaje='Introducir datos correctamente',reg=formulario_nuevo_estado.render())
			...
			api = tweepy.API(auth)
			api.update_status(i.estado_twitter)

			return plantillas.nuevo_estado_twitter(mensaje='Nuevo estado actualizado',reg=formulario_nuevo_estado.render())

class buscar_contenido_twitter:
	def GET(self):
		usuario = comprueba_usuario()
		formulario_buscar_contenido = form_twitter2()
		web.header('Content-Type', 'text/html; charset=utf-8')
		return plantillas.buscar_contenido(mensaje='',reg=formulario_buscar_contenido.render())

	def POST(self):
		usuario = comprueba_usuario()
		formulario_buscar_contenido = form_twitter2()

		if not formulario_buscar_contenido.validates():
			web.header('Content-Type', 'text/html; charset=utf-8')
			return plantillas.buscar_contenido(mensaje='Introducir datos correctamente',reg=formulario_buscar_contenido.render())
			...
			return plantillas.buscar_contenido2(var1=var_aux)

class mostrar_mis_tweets:
	...
	def GET(self):
			...
			for tweets in x.user_timeline():
				dtwitter = {"fecha": tweets.created_at,"texto": tweets.text, "salto": '\n *'*38}
				var_aux.append(dtwitter)

			return plantillas.mostrar_mis_tweets(var1=var_aux)
